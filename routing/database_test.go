...
func Test_querySeeds(t *testing.T){
	...
	node = &tNode{Addr:"na",ID:[64]byte{7,63,74},AddTime:time.Now().Unix()}
	err = db.updateNode(node)
	if err != nil {
		t.Error(err)
	}
	//want[node.ID] = struct{}{}
	node = &tNode{Addr:"nb",ID:[64]byte{4,26,84},AddTime:time.Now().Unix()}
	err = db.updateNode(node)
	if err != nil {
		t.Error(err)
	}
	want[node.ID] = struct{}{}
	node = &tNode{Addr:"nc",ID:[64]byte{10,14,24},AddTime:time.Now().Unix()}
	err = db.updateNode(node)
	if err != nil {
		t.Error(err)
	}
	want[node.ID] = struct{}{}
	err = db.updateLastPongReceived([64]byte{7,63,74},time.Now())
	err = db.updateLastPongReceived([64]byte{4,26,84},time.Now())
	err = db.updateLastPongReceived([64]byte{10,14,24},time.Now())
